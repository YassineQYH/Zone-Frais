...
class CartController extends AbstractController
{
    ...
    public function index(Cart $cart, CategoryRepository $category, WeightRepository $weight)
    {
        ...
        /* dd($priceList); */
        /* dump(getType($priceList[(string)$poid]));
        dd($priceList[(string)$poid]); */

        /* dump($priceList);
        dd($poid); */

        if(is_string($priceList[(string)$poid])){
            $totalLivraison=$priceList[(string)$poid];
        } else
        $totalLivraison=$priceList[(double) $poid];
    ...
    public function fillPriceList($weight){
        ...
        foreach($weight as $item){
           ...
           if($item->getKg() === 0.25 or $item->getKg() === 0.50 or $item->getKg() === 0.75){
